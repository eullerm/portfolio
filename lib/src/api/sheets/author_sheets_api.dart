...
''';
  ...
  static Map<String, int> worksheets = {'author': 815127933};

  static Future init() async {
    try {
      Spreadsheet spreadsheet = await _gsheets.spreadsheet(_spreadsheetId);
      _authorSheet = await _getWorkSheet(spreadsheet, page: worksheets['author']!);
    } catch (e) {
      print('Init error: $e');
    }
  }
  ...
  static Future<Author?> getByLanguage(String language) async {
    ...
    final json = await _authorSheet!.cells.map.rowByKey(language, fromColumn: 1);

    return json == null ? null : Author.fromMapCell(json);
  }
}
